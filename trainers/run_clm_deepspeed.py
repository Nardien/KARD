...
def main():
    ...
    print(f"Before Tokenizer Length: {len(tokenizer)}")
    if tokenizer.pad_token_id is None:
        print('Adding [PAD] token to tokenizer and model word embeddings.')
        num_added_tokens = tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        embedding_layer = model.resize_token_embeddings(len(tokenizer))
        config.pad_token_id = tokenizer.pad_token_id
    print(f"After Tokenizer Length: {len(tokenizer)}")

    train_dataset = load_from_disk(os.path.join(data_args.dataset_name, "train"))
    eval_dataset = load_from_disk(os.path.join(data_args.dataset_name, "eval"))
